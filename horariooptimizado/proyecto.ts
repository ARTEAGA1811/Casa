...
*/
  ...
  function OrdenarPorMaterias(){
   // const Datos =  await EntradaDeDatos();
    const Datos = [ { Materia: 'PROBABILIDAD',
    Curso: 'GR3',
    Dias: [ 'Martes', 'Jueves' ],
    Horas: [ '14-16', '14-16' ] },
  ...
  { Materia: 'SISTEMAS OPERATIVOS',
    ...
    let SoloLasMateriasCasi = Datos
      .map(
        function(ValorActualUno){
            const nuevoObjetoUno = {
    ...
    const SoloLasMaterias = []
    for(let i = 0; i<SoloLasMateriasCasi.length; i++){
